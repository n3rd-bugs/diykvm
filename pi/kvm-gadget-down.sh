...
for l in "$G"/configs/c.1/hid.usb0 "$G"/configs/c.1/hid.usb1 "$G"/configs/c.1/hid.usb2 "$G"/configs/c.1/mass_storage.usb0 "$G"/configs/c.1/acm.usb0; do [ -L "$l" ] && rm -f "$l"; done
...
[ -d "$G/functions/hid.usb2" ] && rmdir "$G/functions/hid.usb2"
