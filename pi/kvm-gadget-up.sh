...
#   mouse=true        /dev/hidg1  mouse (Report-ID multiplexed)                                     (1 IN)
#                       abs=[0x02][btns][Xlo][Xhi][Ylo][Yhi][wheel]  rel=[0x03][btns][dx][dy][wheel]
...
# Each HID is IN-endpoint-only (no_out_endpoint=1). The two HID functions are ALWAYS created so /dev/hidg
# numbering stays stable (kbd=hidg0, mouse=hidg1); they are only LINKED into the config when enabled, and
# an unlinked function uses no endpoints.
set -e
...
MOUSE=$(cfg_bool mouse true)           # HID mouse          -> /dev/hidg1
...
if [ "$KBD" != "true" ] && [ "$MOUSE" != "true" ] && [ "$MSD" != "true" ] && [ "$ACM" != "true" ]; then
...
# hid.usb0 (keyboard) + hid.usb1 (mouse): ALWAYS created so /dev/hidg numbering is stable (kbd=hidg0,
# mouse=hidg1); LINKED into the config below only when enabled. An unlinked function uses no endpoints.
...
# Config + link the ENABLED functions (link order sets /dev/hidg numbering: kbd=hidg0, mouse=hidg1)
...
echo "GADGET_UP udc=$UDC kbd=$KBD mouse=$MOUSE msd=$MSD acm=$ACM"
