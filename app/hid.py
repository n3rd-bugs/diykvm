"""HID report translation for the DIY PiKVM.

Writes report-ID-multiplexed frames to a single /dev/hidg0:
  keyboard = [0x01, mods, 0x00, k1..k6]      (9 bytes)
  mouse    = [0x02, btns, Xlo,Xhi, Ylo,Yhi, wheel]  (7 bytes, X/Y absolute 0..32767)
"""
import os
import threading
...
HIDG = "/dev/hidg0"
KBD_ID = 0x01
MOUSE_ABS_ID = 0x02   # desktop: absolute positioning ([id][btn][Xlo Xhi][Ylo Yhi][wheel])
MOUSE_REL_ID = 0x03   # touch:   relative/trackpad     ([id][btn][dx][dy][wheel])
ABS_MAX = 32767
...
class HIDController:
    def __init__(self, path: str = HIDG):
        self.path = path
    ...
    def _kbd_report(self) -> bytes:
        ...
        return bytes([KBD_ID, self._mods, 0x00, *keys])
    ...
    def button(self, index: int, down: bool):
        ...
        # works for both desktop (positioned by move_abs) and touch (positioned by move_rel).
        ...
        self._write(self._rel_report())

    def wheel(self, dy: int):
        dy = max(-127, min(127, dy))
        self._write(self._rel_report(wheel=dy))
        self._write(self._rel_report(wheel=0))   # auto-release the wheel axis
